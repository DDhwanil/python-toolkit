...
def check():
    ...
    for line in datafile:        
        ...
        count=count+1
    return found
print("Enter 1 for Network scanning")
print("Enter 2 for Networking capturing")
print("Enter 3 for Linux Info")
...
if fil=='1':    
    ...
    if fil2=='16':
        ...
        os='OS:' 
        o=subprocess.check_output(["nmap","-sC",IP])           
        if o.find(os):
            w=o.find(os)
            e=o.find('Computer name')
            print o[w:e-5]                          
    if fil2=='17':
        IP=raw_input('\nEnter the IP :')
...
if fil=='3':
        ...
        print("Enter 17 to display ACCEPT log of Apache2")
        ...
        if fil2=='13':
            subprocess.call(["cat","/etc/gshadow"])
        if fil2=='14':
            subprocess.call(["cat","/etc/services"])
        if fil2=='15':
            subprocess.call(["sudo","/sbin/fdisk","-l"])
        if fil2=='16':
            subprocess.call(["df","-h"])
        if fil2=='17':
            subprocess.call(["cat","/var/log/apache2/ACCEPT.log"])    
...
if fil=='4':
    ...
    if fil2=='3':
        ...
        if fil3=='3':
            ...
            fil4=raw_input("Enter the number :")
            if fil4=='1':
                print("Enter 3 to set firewall default to allow all incomming connections")
                print("Enter 4 to set firewall default to deny all incomming connections")
                print("Enter 5 to set firewall default to allow all outgoing connections")
                print("Enter 6 to set firewall default to deny all outgoing connections")
                print("Enter 7 to create or delete rule for allow incomming connection for port")
                print("Enter 8 to allow incomming connection for port with protocol")
                print("Enter 9 to allow deny connection for port")
                print("Enter 10 to allow deny connection for port with protocol")
                print("Enter 11 for allow or deny IP")
                print("Enter 12 to allow or deny  IP with port number")
